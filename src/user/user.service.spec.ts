...
describe('UserService', () => {
  ...
  it("shouldn't valid : Bad mail", async () => {
    mockUser.email = 'a';
    await expect(service.isValid(mockUser)).rejects.toThrow(
      Constants.ERROR_MSG_IS_EMAIL,
    );
  });
  ...
  it("shouldn't valid : Bad length password", async () => {
    ...
    await expect(service.isValid(mockUser)).rejects.toThrow(
      Constants.ERROR_MSG_LENGTH_PWD,
    );
  });
  ...
  it("shouldn't valid : lastname doesn't exist", async () => {
    mockUser.firstname = '';
    await expect(service.isValid(mockUser)).rejects.toThrow(
      Constants.ERROR_MSG_IS_NOT_EMPTY,
    );
  });
  ...
  it("shouldn't valid : Bad birthdate - far too young", async () => {
    ...
    await expect(service.isValid(mockUser)).rejects.toThrow(
      Constants.ERROR_MSG_USER_WRONG_AGE,
    );
  });
